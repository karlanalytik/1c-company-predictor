...
METRICS_PATH = f"{BASE_PATH}/artifacts/metrics_xgboost_simple.csv"
...
def read_gold_from_s3():
    return wr.s3.read_parquet(GOLD_PATH, columns=GOLD_COLUMNS)


@st.cache_data(ttl=300)
def read_forecast_from_s3():
    return wr.s3.read_parquet(FORECAST_PATH, columns=FORECAST_COLUMNS)
